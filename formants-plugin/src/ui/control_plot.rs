...
pub struct ControlPlot {
    vowels_map: Vec<(String, Option<[f32; 2]>)>,
    hovered_control_point_index: usize,
}

impl ControlPlot {
    pub fn new(vowels_map: Vec<(String, Option<[f32; 2]>)>) -> Self {
        Self {
            vowels_map: vowels_map,
        ...
        egui::Frame::group(ui.style())
            ...
            .show(ui, |ui| {
                ...
                let plot_response = plot.show(ui, |plot_ui| {
                    ...
                    for (vowel, xy) in self.vowels_map.as_slice() {
                        if let Some([x, y]) = xy {
                ...
                if plot_response.response.is_pointer_button_down_on() {
                    if self.hovered_control_point_index == usize::MAX {
                        if let Some(hovered_control_point_index) =
                            item_id_to_control_point_index(&plot_response.hovered_plot_item)
                        {
                            self.hovered_control_point_index = hovered_control_point_index;
                        }
                    }
                    if params.path.enabled.value() {
                        if self.hovered_control_point_index < usize::MAX
                        ...
                        {
                            params.path.control_points[self.hovered_control_point_index].set(
                    ...
                    self.hovered_control_point_index = usize::MAX
                }
            });
    }
}
...
pub fn make_vowels_map(
    frequency_transform: &params::FrequencyTransform,
) -> Vec<(String, Option<[f32; 2]>)> {
    vec![
        (
            String::from("U"),
            frequency_transform.xy_from_frequencies([320_f32, 800_f32]),
        ),
        (
            String::from("O"),
            frequency_transform.xy_from_frequencies([500_f32, 1000_f32]),
        ),
        (
            String::from("A"),
            frequency_transform.xy_from_frequencies([1000_f32, 1400_f32]),
        ),
        (
            String::from("Ö"),
            frequency_transform.xy_from_frequencies([500_f32, 1500_f32]),
        ),
        (
            String::from("Ü"),
            frequency_transform.xy_from_frequencies([320_f32, 1650_f32]),
        ),
        (
            String::from("Ä"),
            frequency_transform.xy_from_frequencies([800_f32, 2000_f32]),
        ),
        (
            String::from("E"),
            frequency_transform.xy_from_frequencies([470_f32, 2300_f32]),
        ),
        (
            String::from("I"),
            frequency_transform.xy_from_frequencies([260_f32, 3200_f32]),
        ),
    ]
}
